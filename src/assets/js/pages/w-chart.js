...
function floatchart() {
  ...
  (function () {
    ...
    var options_performance = {
      ...
      labels: ['Ирсэн', 'Өвчтэй', 'Чөлөөтэй', 'Тасалсан'],
      fill: {
        opacity: [1, 1, 1, 0.3]
      },
      legend: {
    ...
    var options_spider_chart = {
      ...
      series: [{
        ...
        name: 'Term 2',
        data: [50, 55, 45, 60, 50, 65]
      }, {
        name: 'Term 3',
        data: [60, 50, 65, 30, 60, 40]
      }],
      labels: ['Math', 'Science', 'English', 'History', 'Geography', 'Art']
    };
    var chart_spider = new ApexCharts(document.querySelector('#spider-chart'), options_spider_chart);
    chart_spider.render();
    ...
    let combinedTermChart = null;
    let spiderChart = null;

    courseSelect.addEventListener('change', function () {
      const selectedCourse = courseSelect.value;
      if (selectedCourse) {
        updateCombinedTermChart(selectedCourse);
        updateSpiderChart(selectedCourse);
      }
    });

    function updateCombinedTermChart(course) {
      const combinedData = {
        'Term 1': courseData[course]['Term 1'],
        'Term 2': courseData[course]['Term 2'],
        'Term 3': courseData[course]['Term 3']
      };

      const combinedSeries = Object.entries(combinedData).map(([term, data]) => ({
        name: term,
        data: data
      }));

      const options_combined_term_chart = {
        chart: {
          type: 'area',
          height: 350,
          toolbar: {
        ...
        xaxis: {
          categories: [].concat(terms['Term 1'], terms['Term 2'], terms['Term 3']),
          labels: {
            hideOverlappingLabels: true,
          },
          axisBorder: {
      ...
      if (combinedTermChart) {
        combinedTermChart.destroy();
      }

      combinedTermChart = new ApexCharts(document.querySelector('#combined-term-chart'), options_combined_term_chart);
      combinedTermChart.render();
    }
